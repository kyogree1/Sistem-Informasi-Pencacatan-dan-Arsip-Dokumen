            ...
            <div class="rounded-2xl bg-white shadow-lg shadow-slate-200/50 border border-slate-100 p-5">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <form method="GET" action="{{ route('admin.pegawai.index') }}" class="flex gap-2 w-full sm:w-auto">
                        ...
                        <button type="submit" class="rounded-xl bg-[#12284B] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#0f3a6e] transition shadow-sm">Cari</button>
                    </form>
                    <a href="{{ route('admin.pegawai.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#12284B] via-[#0055A0] to-[#438BC4] px-5 py-2.5 text-sm font-semibold text-white hover:shadow-lg transition-all shadow-sm">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead class="bg-[#8CC1E9] border-b border-[#8CC1E9] whitespace-nowrap">
                            <tr>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Personal Number</th>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Nama</th>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Departemen</th>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Divisi</th>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Password</th>
                                <th class="px-6 py-3.5 text-center text-[11px] font-bold text-[#12284B] uppercase tracking-wider whitespace-nowrap">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                            @forelse ($pegawai as $user)
                                <tr class="hover:bg-sky-50/50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-mono font-bold text-[#12284B] group-hover:bg-sky-100 group-hover:text-sky-700 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="text-sm font-semibold text-[#12284B]">{{ $user->name }}</div>
                                                <div class="text-xs text-slate-400">{{ $user->email }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-medium whitespace-nowrap">
                                        {{ $user->departemen ?? '-' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600 font-medium whitespace-nowrap">
                                        {{ $user->divisi ?? '-' }}
                                    ...
                                    <td class="px-6 py-4 text-center whitespace-nowrap whitespace-nowrap">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-14 text-center whitespace-nowrap">
            ...
            <div x-show="showEdit" x-transition class="w-full max-w-lg mx-4 rounded-2xl bg-white shadow-2xl overflow-hidden">
                ...
                <form :action="editAction" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-1">Departemen</label>
                            <input type="text" name="departemen" x-model="editData.departemen" required class="w-full rounded-xl border-slate-200 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-sm">
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-3 pt-3 border-t border-slate-100">
                        ...
                        <button type="submit" class="rounded-xl bg-[#12284B] px-5 py-2 text-sm font-semibold text-white hover:bg-[#0f3a6e] transition shadow-sm">Perbarui</button>
                    </div>
                </form>
            </div>
        </div>
