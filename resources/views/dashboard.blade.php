    ...
    <div class="py-0">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 -mt-6 relative z-20 pb-10" x-data="{ showGuide: false }">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                <!-- Total Arsip -->
                <div class="group rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:border-sky-200 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <p class="text-3xl font-extrabold text-[#12284B] dark:text-slate-100">{{ $totalArsip }}</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Total Arsip Tersimpan</p>
                </div>

                <!-- Arsip Bulan Ini -->
                <div x-data="{ monthOpen: false, yearOpen: false }" class="relative group rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:border-emerald-200 transition-all duration-300">
                    ...
                    <p class="text-3xl font-extrabold text-[#12284B] dark:text-slate-100 mt-2">{{ $arsipBulanIni }}</p>
                    <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 font-medium leading-tight">
                        @php
                ...
                <!-- Akun Aktif Sejak -->
                <div class="group rounded-2xl bg-white dark:bg-slate-900 p-5 shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 hover:shadow-xl hover:border-violet-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between mb-4">
                    ...
                    <p class="text-lg font-extrabold text-[#12284B] dark:text-slate-100">{{ auth()->user()->created_at->format('d M Y') }}</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Akun Terdaftar Sejak</p>
                </div>
            ...
            <div class="mb-6">
                <button @click="showGuide = !showGuide"
                    class="inline-flex items-center gap-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2.5 text-sm font-semibold text-[#12284B] dark:text-slate-100 hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-sky-200 transition shadow-sm">
            ...
            <div x-show="showGuide" x-transition class="mb-8 rounded-2xl border border-sky-200 dark:border-slate-700 bg-gradient-to-r from-sky-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-bold text-[#12284B] dark:text-slate-100 flex items-center gap-2">
                    ...
                    <button @click="showGuide = false" class="text-xs text-sky-600 hover:text-sky-800 font-semibold bg-white dark:bg-slate-900 rounded-lg px-3 py-1 border border-sky-200 dark:border-slate-700 hover:bg-sky-50 dark:hover:bg-slate-800 transition">Tutup</button>
                </div>
                <ol class="list-decimal ml-5 space-y-2 text-[#12284B] dark:text-slate-200 text-sm">
                ...
                <div class="rounded-2xl bg-white dark:bg-slate-900 shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
                    <table class="table-base">
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-slate-800">
                                ...
                                <tr class="table-row group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-semibold text-[#12284B] dark:text-slate-100">{{ $arsip->nama }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs font-medium text-slate-600 dark:text-slate-300">
                            ...
                            @empty
                                <tr><td colspan="2" class="px-6 py-14 text-center text-sm text-slate-400 dark:text-slate-500 whitespace-nowrap">Belum ada arsip terbaru</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
            ...
            <!-- WARNING -->
            <div class="rounded-2xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/30 dark:to-orange-900/30 border border-amber-200 dark:border-amber-700/50 p-5 flex items-start gap-3">
                ...
                <p class="text-sm text-amber-800 dark:text-amber-200"><strong>Perhatian:</strong> Dokumen dalam sistem ini bersifat internal dan rahasia. Dilarang menyebarluaskan tanpa izin resmi dari BPD Kaltim Kaltara.</p>
            </div>
        </div>
    </div>
</x-app-layout>
