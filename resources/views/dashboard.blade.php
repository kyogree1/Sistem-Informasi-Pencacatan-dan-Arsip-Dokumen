<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-xl text-[#0C2D57] leading-tight">Dashboard</h2>
    </x-slot>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sky-200 text-sm ml-[52px]">Sistem Informasi Pencatatan dan Arsip Dokumen — BPD Kaltim Kaltara</p>
                    </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 -mt-6 relative z-20 pb-10" x-data="{ showGuide: false }">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                ...
                <div class="group rounded-2xl bg-white p-5 shadow-lg shadow-slate-200/50 border border-slate-100 hover:shadow-xl hover:border-sky-200 transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-[10px] font-bold text-blue-500 bg-blue-50 rounded-full px-2 py-0.5 uppercase">Arsip</span>
                    </div>
                    <p class="text-3xl font-extrabold text-[#0C2D57]">{{ $totalArsip }}</p>
                ...
                <div class="group rounded-2xl bg-white p-5 shadow-lg shadow-slate-200/50 border border-slate-100 hover:shadow-xl hover:border-emerald-200 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <p class="text-3xl font-extrabold text-[#0C2D57]">{{ $arsipBulanIni }}</p>
                ...
                <div class="group rounded-2xl bg-white p-5 shadow-lg shadow-slate-200/50 border border-slate-100 hover:shadow-xl hover:border-violet-200 transition-all duration-300 hover:-translate-y-1">
                    ...
                    <p class="text-lg font-extrabold text-[#0C2D57]">{{ auth()->user()->created_at->format('d M Y') }}</p>
                    <p class="text-xs text-slate-500 mt-1 font-medium">Akun Terdaftar Sejak</p>
                </div>

                <!-- Quick Action -->
                <a href="{{ route('archive.create') }}" class="group rounded-2xl bg-gradient-to-br from-[#0C2D57] to-[#0c4a8a] p-5 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 relative overflow-hidden">
                    <div class="absolute -right-4 -bottom-4 w-24 h-24 rounded-full bg-white/5"></div>
                    <div class="absolute -right-2 -top-2 w-16 h-16 rounded-full bg-white/5"></div>
                    <div class="flex items-center justify-between mb-4 relative z-10">
                        ...
                        <svg class="h-5 w-5 text-white/40 group-hover:text-white/70 group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
                    </div>
                    <p class="text-lg font-bold text-white relative z-10">Input Arsip</p>
                    <p class="text-xs text-sky-300 mt-1 font-medium relative z-10">Tambah dokumen baru</p>
                </a>
            </div>
            ...
            <div class="mb-6">
                <button @click="showGuide = !showGuide"
                    class="inline-flex items-center gap-2 rounded-xl bg-white border border-slate-200 px-4 py-2.5 text-sm font-semibold text-[#0C2D57] hover:bg-slate-50 hover:border-sky-200 transition shadow-sm">
            ...
            <div x-show="showGuide" x-transition class="mb-8 rounded-2xl border border-sky-200 bg-gradient-to-r from-sky-50 to-blue-50 p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base font-bold text-[#0C2D57] flex items-center gap-2">
                ...
                <ol class="list-decimal ml-5 space-y-2 text-[#0C2D57] text-sm">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-lg font-bold text-[#0C2D57]">Arsip Terbaru</h2>
                    </div>
                    ...
                    <table class="min-w-full divide-y divide-slate-100">
                        <thead>
                            <tr class="bg-gradient-to-r from-slate-50 to-slate-100/50">
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Nama Dokumen</th>
                                <th class="px-6 py-3.5 text-left text-[11px] font-bold text-slate-500 uppercase tracking-wider">Tanggal</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-50">
                                ...
                                <tr class="hover:bg-sky-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-sm font-semibold text-[#0C2D57]">{{ $arsip->nama_debitur }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                            ...
                            @empty
                                <tr><td colspan="2" class="px-6 py-14 text-center text-sm text-slate-400">Belum ada arsip terbaru</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
